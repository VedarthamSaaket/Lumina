...
from models.database import get_db, User, CrisisLog, ScreeningResult, CognitiveTestResult, MoodLog, JournalEntry, CBTProgress, Conversation, ChatMessage
from utils.auth import get_current_user
from services.llm import chat_with_llama, chat_with_llama_stream, detect_risk_level, post_process_response, build_user_context
...
# Replace the _get_user_context function in backend/routers/chat.py with this:
# backend/routers/chat.py
# Only _get_user_context needs updating - everything else is unchanged.
# Replace your existing _get_user_context with this version.
...
def _get_user_context(user_id: str, db: Session) -> str:
    try:
        ...
        # Memory game history - now includes full raw_results (round-by-round)
        ...
        for r in memory_rows:
            try:
                scores = _json.loads(r.score_data) if r.score_data else {}
            except Exception:
                scores = {}
            try:
                rounds = _json.loads(r.raw_results) if r.raw_results else []
            except Exception:
                rounds = []
            memory_history.append({
                ...
                "scores":       scores,
                "rounds":       rounds,   # full sequence-level data
            })
        ...
        return build_user_context(
            ...
            memory_history=memory_history,       # now rich with rounds + scores
...
def create_conversation(data: NewConversationRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
async def stream_message(
    ...
    """Streaming chat endpoint - yields SSE tokens as Mistral generates them."""
    print(f"[Chat/stream] From {current_user.id}: {data.content[:40]}")
    ...
    # Crisis level 3 - no LLM, immediate hard-coded response
    if risk_level >= 3:
        async def crisis_stream():
            msg = "Your safety is the only thing that matters right now. Please call emergency services (112 or 911) immediately, or contact iCall at 9152987821. You are not alone."
    ...
    # Build history from DB (source of truth, not client-supplied history)
    history_rows = (
        db.query(ChatMessage)
    ...
    async def generate():
        ...
        try:
            ...
            print(f"[Chat/stream] Generator error: {type(e).__name__}: {e}")
            error_msg = "Something went wrong mid-response. Please try again."
            yield f"data: {json.dumps({'text': error_msg})}\n\n"
            full_response = full_response or error_msg

        # Post-process: strip TOOL_REDIRECT from saved content, parse it out
        tool_redirect = None
        ...
        # Send metadata frame - frontend uses this to update conversation_id + risk
...
async def send_message(
    ...
    """Non-streaming fallback endpoint - returns full response at once."""
    print(f"[Chat] From {current_user.id}: {data.content[:40]}")
    ...
    if risk_level >= 3:
        return {
            "response": "Your safety is the only thing that matters right now. Please call emergency services (112 or 911) immediately, or contact iCall at 9152987821. You are not alone.",
